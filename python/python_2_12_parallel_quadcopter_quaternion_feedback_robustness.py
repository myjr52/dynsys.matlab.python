...
#-----------------------------------------------------------------------------
# Make it a function for Monte-Carlo Simulation
#-----------------------------------------------------------------------------
def robustness_analysis_MC(MC_id):
    ...
    print(f'#{MC_id}: {np.linalg.norm(dJ):6.5f}, {q13_ts:4.2f}\n')
...
ts_all = np.array([i[0] for i in result])
dJ_norm_all = np.array([i[1] for i in result])
