...
delta_best_ABC = delta_best_C;
time_exp = time_C;
Enzy_exp = Enzy_C;

delta_dim = length(delta_best_ABC);
delta_norm_all = zeros(1,num_MC);
j_cost_all  = zeros(1,num_MC);

for idx=1:num_MC

    idx
    
    delta_robust = 2*(rand(1,delta_dim)-0.5);
    delta_current = delta_best_ABC(:)'.*(1+0.05*delta_robust(:)');

    j_cost = Santillan_Model_Fit_Cost(delta_current, time_Text, time_exp, Enzy_exp, false);

    delta_norm_all(idx) = norm(delta_robust);
    j_cost_all(idx) = j_cost;
    
end
figure();
plot(delta_norm_all,j_cost_all,'.');
